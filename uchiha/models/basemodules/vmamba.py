...
class VSSBlock(nn.Module):
    ...
    Args:
        ...
        ssm_drop_rate (): SS2D的dropout
        ssm_init (): SS2D的初始化版本
        forward_type (): 前向传播的版本
        mlp_ratio (): MLP隐藏层维度
        mlp_act_layer (): MLP的激活函数
        mlp_drop_rate (): MLP的dropout
        gmlp (): 是否使用GMLP
        use_checkpoint ():
        post_norm ():
        _SS2D ():
