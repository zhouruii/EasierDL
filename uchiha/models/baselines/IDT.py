...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size, num_heads, qkv_bias=True, qk_scale=None, k_bound=8):
        ...
        self.Position_Extract = nn.Sequential(
            torch.nn.Conv2d(dim, dim, kernel_size=kernelsize, stride=1, padding=padding,
                            groups=dim),
        ...
        relative_coords = torch.clamp(coords_flatten[:, :, None] - coords_flatten[:, None, :], -k_bound + 1,
                                      k_bound - 1)  # 2, Wh*Ww, Wh*Ww
...
class SpatialTransformerLayer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = x.transpose(1, 2).contiguous().view(B, C, H, W)
        x2 = torch.nn.functional.unfold(x, self.win, stride=self.win).reshape(B, C, self.win * self.win, nw).reshape(
            B * C, self.win * self.win, nw)  # [BC,nw, W^2]
        x2 = x2.transpose(1, 2)
...
if __name__ == '__main__':
    ...
    net = IDT(img_size=128, in_chans=305, embed_dim=48)
    out = net(img)
    print(out.size())
