...
class FAConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, padding=0, deploy=False,
        ...
        if deploy:
            ...
            self.square_point_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1,
...
class AACNet(nn.Module):
    def __init__(self, in_dim=150, out_dim=150, dim=64, kernel_size=3, padding=1, num_blocks=5, stride=1, deploy=False):
...
if __name__ == '__main__':
    x = torch.randn(32, 150, 64, 64).cuda()
    test_kernel_padding = [(3, 1), (5, 2), (7, 3), (9, 4), (11, 5)]
    mcplb = AACNet(in_dim=150, out_dim=150, kernel_size=3, padding=1, stride=1, num_blocks=5).cuda()
    out = mcplb(x)
