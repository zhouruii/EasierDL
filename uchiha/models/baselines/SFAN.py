...
class SFAN(nn.Module):
    def __init__(self,
        ...
        self.first = nn.Conv2d(in_channels, dim, 3, 1, 1)
        ...
        self.encoder_level2 = nn.Sequential(
            *[MEB(in_size=int(dim * 2 ** 1), out_size=int(dim * 2 ** 1)) for i in range(num_blocks[1])])

        self.down2_3 = Downsample(int(dim * 2 ** 1))  ## From Level 2 to Level 3

        self.encoder_level3 = nn.Sequential(
            *[MEB(in_size=int(dim * 2 ** 2), out_size=int(dim * 2 ** 2)) for i in range(num_blocks[2])])

        self.down3_4 = Downsample(int(dim * 2 ** 2))  ## From Level 3 to Level 4
        self.latent = nn.Sequential(
            *[MEB(in_size=int(dim * 2 ** 3), out_size=int(dim * 2 ** 3)) for i in range(num_blocks[3])])
        ...
        self.decoder_level1 = nn.Sequential(
            *[MEB(in_size=int(dim * 2 ** 1), out_size=int(dim * 2 ** 1)) for i in range(num_blocks[0])])
        ...
        if self.decoder:
            self.fre1 = DFLB(dim * 2 ** 3, num_heads=heads[2], bias=bias, in_dim=in_channels)
            self.fre2 = DFLB(dim * 2 ** 2, num_heads=heads[2], bias=bias, in_dim=in_channels)
            self.fre3 = DFLB(dim * 2 ** 1, num_heads=heads[2], bias=bias, in_dim=in_channels)
