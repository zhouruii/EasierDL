...
class FPro(nn.Module):
    def __init__(self,
        ...
        self.prompt_d3 = PromptModule(basic_dim=dim, dim=int(dim * 2 ** 2), input_resolution=32)
        ...
        self.prompt_d2 = PromptModule(basic_dim=dim, dim=int(dim * 2 ** 1), input_resolution=64)
        ...
        self.prompt_d1 = PromptModule(basic_dim=dim, dim=int(dim * 2 ** 1), input_resolution=128)

        self.refinement = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type, isAtt=True) for i in
            range(num_refinement_blocks)])
        self.prompt_r = PromptModule(basic_dim=dim, dim=int(dim * 2 ** 1), input_resolution=128)
