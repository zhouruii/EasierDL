...
class AIDTransformer(nn.Module):
    def __init__(self, img_size=128, in_channels=3,
        ...
        # Input/Output
        self.input_proj = InputProj(in_channel=in_channels, out_channel=embed_dim, kernel_size=3, stride=1,
                                    act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj(in_channel=3 * embed_dim, out_channel=in_channels, kernel_size=3, stride=1)
        self.output_proj1 = OutputProj(in_channel=embed_dim, out_channel=in_channels, kernel_size=3, stride=1)
        # Encoder
