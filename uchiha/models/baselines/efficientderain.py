...
class KPN(nn.Module):
    def __init__(self, in_channels=3, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
                 channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        super(KPN, self).__init__()
        self.upMode = upMode
        self.burst_length = burst_length
        self.core_bias = core_bias
        self.color_channel = in_channels if color else 1
        in_channel = (in_channels if color else 1) * (burst_length if blind_est else burst_length + 1)
        out_channel = (in_channels if color else 1) * (
            2 * sum(kernel_size) if sep_conv else np.sum(np.array(kernel_size) ** 2)) * burst_length
        if core_bias:
            out_channel += (in_channels if color else 1) * burst_length
        # 各个卷积层定义
        # 2~5层都是均值池化+3层卷积
