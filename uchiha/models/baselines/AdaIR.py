...
class AdaIR(nn.Module):
    def __init__(self, 
        ...
        if self.decoder:
            self.fre1 = FreModule(dim*2**3, num_heads=heads[2], bias=bias, in_dim=in_channels)
            self.fre2 = FreModule(dim*2**2, num_heads=heads[2], bias=bias, in_dim=in_channels)
            self.fre3 = FreModule(dim*2**1, num_heads=heads[2], bias=bias, in_dim=in_channels)
