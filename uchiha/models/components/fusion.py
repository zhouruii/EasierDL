...
class CatConv(nn.Module):
    def __init__(self, in_channel):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channel, in_channel // 2, kernel_size=1, stride=1, padding=0, bias=False),
            nn.GELU(),
            nn.Conv2d(in_channel // 2, in_channel // 2, kernel_size=1, stride=1, padding=0, bias=False),
            nn.GELU()
        )
...
class CatConvs(nn.Module):
    def __init__(self, in_channels):
        ...
        for i in range(self.num_fusions):
            self.fusions.append(CatConv(self.in_channels[i]))
